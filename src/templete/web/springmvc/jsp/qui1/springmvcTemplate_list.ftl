  ...
  <head>
	...
	function del(){
		...
		if(id==0){
			...
			top.Dialog.confirm("您确定要删除这条信息",
					function(){
						location="<%=path%>/${p.lowerName}Controller/deleteById.do?id="+id;
					},
					function(){
	...
	function edit(){
		...
		if(id==0){
			...
			location="<%=path%>/${p.lowerName}Controller/editById.do?id="+id;
		}
	}
	//查看
	function view(){
		...
		if(id==0){
			...
			location="<%=path%>/${p.lowerName}Controller/findById.do?id="+id;
		}
	}
	function query()
	{
	...
	<div id="scrollContent"   style="overflow-x:hidden;">
		...
		<table class="tableStyle" mode="list" useRadio="true" id="radioTable" >
			...
			<tr>
			 <#list p.gci_columns as u>
			 	<#if (u.clumn_name=='id')> 
					<td><input type="radio" name="id" value="${"${"}vo.${u.clumn_name}}"/></td>
				<#else> 
					<#if (u.dataType=="Date")>
					<td><fmt:formatDate value="${"${"}vo.${u.clumn_name}}" type="date"/></td>
					<#else>
